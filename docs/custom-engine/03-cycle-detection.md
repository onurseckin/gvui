...
When a DFS encounters a node that it is *already exploring* (an ancestor in the current path), this is called a **back edge**.
...
If we treat every SCC as a single giant "super-node", the resulting graph will NEVER have cycles. This is called a *condensation graph*. 
...
2. **Low-Link Value (lowlink):** This is the magic number. It represents the *smallest discovery time* reachable from this node, including through its children and back edges. 
...
However, Tarjan's algorithm only *finds* the cycles. It doesn't fix them. For a layered layout to work, we must temporarily break these cycles by reversing certain edges. That is the job of the next phase.

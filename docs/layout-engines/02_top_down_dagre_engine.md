...
```
[1. Cycle Removal] -> [2. Layer Assignment (Simplex)] -> [3. Vertex Ordering] -> [4. Coordinate Assignment]
```

### Phase 1: Cycle Removal (Greedy DFS)
Cycles in the graph are identified using a depth-first search (DFS). Back-edges $(u, v)$ where $v$ is an ancestor of $u$ in the DFS tree are temporarily reversed:

$$E_{\text{reversed}} = \{ (u, v) \in E \mid \text{DFS\_Index}(v) < \text{DFS\_Index}(u) \}$$
...
1. Fix layer $L_0$.
2. For $i = 0$ to $k-1$:
   Assign each node $v \in L_{i+1}$ a weight equal to the average X-coordinate of its predecessors in $L_i$:
   $$\text{barycenter}(v) = \frac{1}{|\text{Predecessors}(v)|} \sum_{u \in \text{Predecessors}(v)} x(u)$$
...
Each sub-pass aligns vertices with their median neighbor and builds blocks of connected vertices. The blocks are placed using a block graph compaction algorithm. The final X-coordinate is the average of the 4 candidate alignments:

...
- **35 Graph Topologies**: 26 synthetic stress-test scenarios (`customLayoutScenarios.ts`) + 9 real-world production telemetry graphs (`public/data/graphs/*.json`).
- **Total Test Configurations**: $2 \times 4 \times 35 = 280$ layout executions per audit run.
...
- **Global Leader Line Budget**: Cumulative count of off-edge leader lines across all 280 configurations must not exceed **2**.

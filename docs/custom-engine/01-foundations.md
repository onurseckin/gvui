...
- A **predecessor** is a node that points *to* you.
- A **successor** is a node that you point *to*.
...
Cycles are a nightmare for graph layout. Remember how we said "Sources want to live at the top"? If there is a cycle, *every node in the cycle is reachable from every other node in the cycle*. There is no clear top or bottom anymore. Does `API` go above `DB`, or does `DB` go above `API`? 

...
```typescript
function expand3DNeighbors(curr: AStarNode, grid: RoutingGrid): NeighborTransition[] {
  const neighbors: NeighborTransition[] = [];
  const adjacentKeys = grid.adj.get(curr.vId) ?? [];

  for (const adj of adjacentKeys) {
    const targetPt = grid.vertices.get(adj.targetId)!;
    const moveDir = getSegmentDirection(curr.pt, targetPt);
    const isBend = moveDir !== curr.dir;
    const isHairpin = OPPOSITE_DIR[curr.dir] === moveDir;
...
#### 3. Targeted Sub-Step Pseudocode
```typescript
function computeCandidateCosts(
  curr: AStarNode,
  targetStub: Point,
  trans: NeighborTransition,
  config: LayoutConfig
): { gCost: RouteCost; fCost: RouteCost } {
  const length = curr.gCost.length + trans.stepDistance;
  const bends = curr.gCost.bends + (trans.isBend ? 1 : 0);
  const hairpins = curr.gCost.hairpins + (trans.isHairpin ? 1 : 0);
  const crossings = curr.gCost.crossings;

  const numericG = length + bends * config.penaltyBend + hairpins * config.penaltyHairpin;
  const numericH = Math.abs(curr.pt.x - targetStub.x) + Math.abs(curr.pt.y - targetStub.y);

  const gCost: RouteCost = { crossings, hairpins, bends, length: numericG };
  const fCost: RouteCost = { crossings, hairpins, bends, length: numericG + numericH };
  return { gCost, fCost };
}
```
...
#### 3. Targeted Sub-Step Pseudocode
```typescript
function renderPathWithCrossingBridges(points: Point[], crossings: Point[], r = 6): string {
  let pathStr = `M ${points[0].x} ${points[0].y}`;

  for (let i = 0; i < points.length - 1; i++) {
    const p1 = points[i];
    const p2 = points[i + 1];
    const segCrossings = findCrossingsOnSegment(p1, p2, crossings);
    ...
    if (segCrossings.length === 0) {
      ...
      for (const c of segCrossings) {
        const startX = c.x - r * dx;
        const startY = c.y - r * dy;
        const endX = c.x + r * dx;
        const endY = c.y + r * dy;
        pathStr += ` L ${startX} ${startY} A ${r} ${r} 0 0 0 ${endX} ${endY}`;
      }
      pathStr += ` L ${p2.x} ${p2.y}`;
    }
  }
...
function searchOrthogonalRoute(
  ...
  while (openHeap.size > 0) {
    ...
    const transitions = expand3DNeighbors(current, grid);
    for (const trans of transitions) {
      if (intersectsNodeInterior(current.pt, trans.targetPt, grid.nodeObstacles)) continue;

      const costs = computeCandidateCosts(current, targetPort.stub, trans, config);
      const stateKey = `${trans.targetId}:${trans.moveDir}`;
      const existingG = gCosts.get(stateKey);

      if (!existingG || compareRouteCost(costs.gCost, existingG) < 0) {
        gCosts.set(stateKey, costs.gCost);
        openHeap.push({
          vId: trans.targetId,
          pt: grid.vertices.get(trans.targetId)!,
          dir: trans.moveDir,
          gCost: costs.gCost,
          fCost: costs.fCost,
          parent: current,

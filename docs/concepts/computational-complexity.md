...
Unlike Dagre, our custom engine searches for _perfect_ orthogonal edge routes that rigidly avoid all node obstacles.
...
- **Dagre:** ~2ms.
- **Custom Engine:** ~10ms.
  - _Operation Count:_ The search space is tiny. The engine evaluates maybe 2 layout states. A* explores a 200x200 pixel grid, visiting ~50 grid cells per edge. Total operations: negligible. A perfect layout is found instantly.

### Medium Graph (15 Nodes, 20 Edges)

- **Dagre:** ~8ms.
- **Custom Engine:** ~150ms.
  - _Operation Count:_ The A* router evaluates thousands of grid states to route edges cleanly around the 15 obstacles. The frontier queue grows to evaluate different port assignments. The engine might score 30 different layout permutations.

### Large Graph (80 Nodes, 120 Edges)

- **Dagre:** ~40ms. Stable and reliable.
- **Custom Engine:**
  - _Without limits:_ The engine would evaluate millions of states, attempting to untangle 120 edges. It would take several minutes and crash the browser tab due to out-of-memory errors from the frontier queue.
  - _With our limits (Budget Adaptive):_ ~1200ms, returning a "good enough" layout rather than a perfect one.

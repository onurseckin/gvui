...
| 5   | `left-right` doesn't work                               | The client sends a _fully resolved_ config, so `direction` is always `Some`, and `lib.rs` lets explicit direction win over the mode — the mode's direction is always discarded | **Real bug**                                                |
...
mostly _sideways_ now exits sideways. Still a scored choice over 16 combinations, not a search.
...
Auto-detect _peer_ edges — endpoints sharing a common predecessor with no other directed path
...
This deliberately does _not_ rebuild the lane model on eight directions. The lane model is what makes

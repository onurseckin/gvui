...
export function minimizeCrossings(
  ...
  // Clone layers
  let currentLayers: LayerNode[][] = applyLayerOrderOverrides(layerGraph.layers, layerOrders).map(
    (layer) => [...layer],
  );
  ...
  if (
    bestCrossings === 0 ||
    (layerOrders &&
      (layerOrders instanceof Map ? layerOrders.size : Object.keys(layerOrders).length) > 0)
  ) {

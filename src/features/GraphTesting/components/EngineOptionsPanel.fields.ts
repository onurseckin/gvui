...
const EDGE_FIELDS: FieldSpec[] = [
  ...
  },
  {
    ...
    help: "What a left/right attachment costs, in corners. A side always costs one corner more, so 0 and above keep edges on the flow faces; go negative to use all four sides at the price of more corners and more crossings.",
    min: -4,
    max: 8,
    step: 0.5,
  },
  {
    kind: "number",
    key: "sideFaceCapacity",
    label: "Ports per side face",
    help: "How many edges may attach to one left/right face. Each gets its own descent line, so the face needs more clearance for each. Only has an effect when flow side bias is negative.",
    min: 1,
    max: 6,
    step: 1,
  },
  ...
  {
    kind: "boolean",
    key: "portDestinationAffinity",
    label: "Port destination affinity",
    help: "Places each port next to the node it connects to instead of spreading ports evenly along the face. Shortens the run each edge makes through the routing channel, which cuts both bends and crossings.",
  },
  {
    kind: "boolean",
    key: "crossingAwareLanes",
    label: "Crossing-aware lanes",
    help: "Chooses each edge's depth in a routing channel from the real geometry, so runs are not cut by other edges' drops. The single largest reducer of edge crossings; off restores the older order-space assignment.",
  },
  {
    kind: "number",
    key: "laneOrderMaxSegments",
    label: "Lane order limit",
    help: "Channels with more edges than this keep the unoptimised lane assignment, bounding the optimiser's cost on pathological graphs.",
    min: 64,
    max: 8192,
    step: 64,
    integer: true,
